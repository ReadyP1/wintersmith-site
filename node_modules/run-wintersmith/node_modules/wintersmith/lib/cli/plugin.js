(function() {
  var NpmAdapter, async, cacheDir, chalk, childProcess, clip, commonOptions, displayListing, ensureCacheDir, extend, fetchListing, fileExists, fs, getStorageDir, isPlugin, listFile, loadEnv, loadListing, logLevel, logger, lpad, main, max, maxListAge, mkdirp, normalizePluginName, npm, options, path, readJSON, ref, ref1, updateIfNeeded, usage, waterfall, writeListing,
  ...
  ref = require('./common'), NpmAdapter = ref.NpmAdapter, getStorageDir = ref.getStorageDir, loadEnv = ref.loadEnv, commonOptions = ref.commonOptions;

  ref1 = require('./../core/utils'), fileExists = ref1.fileExists, readJSON = ref1.readJSON, extend = ref1.extend;
  ...
  isPlugin = function(module) {
    return indexOf.call(module.keywords, 'wintersmith-plugin') >= 0;
  };
  ...
  displayListing = function(list, callback) {
    var display, j, left, len, line, maxw, pad, plugin;
    ...
    for (j = 0, len = display.length; j < len; j++) {
      plugin = display[j];
  ...
  main = function(argv) {
    ...
    installPlugin = function(res, callback) {
      var configFile, createPackageJson, env, install, j, len, list, name, p, packageFile, plugin, readConfig, ref2, saveConfig, updateConfig;
      ...
      updateConfig = function(config, callback) {
        ...
        if (ref3 = plugin.name, indexOf.call(config.plugins, ref3) < 0) {
